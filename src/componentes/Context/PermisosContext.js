...
function PermisosProvider({ children }) {
    const { funcionario: { id }, guardarFuncionario, funcionario, loadingFuncionario } = useContext(funcionarioContext2);
    ...
    const [obtenerPermisos, { data, fetchMore }] = useLazyQuery(OBTENER_PERMISOS, {
        pollInterval: 1000
    });
    const [obtenerTotalPermisos, { data: totalPermisosData }] = useLazyQuery(TOTAL_PERMISOS, {
        pollInterval: 1000,
    });
    ...
    }, [id, obtenerPermisos, obtenerTotalPermisos, limite, offset, loading, periodoSeleccionado]);
    ...
    return (
        <Provider value={{ permisos, guardarPermisos, id, guardarFuncionario, totalPermisos, paginador, funcionario,loadingFuncionario }}>
            {children}
        </Provider>
    )
};
