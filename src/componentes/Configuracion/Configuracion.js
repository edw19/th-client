...
export default function Configuracion() {
  ...
  const rangoVacaciones = (e) => {
    ...
    if (e.value[0] && nombre.length <= 8) {
      guardarNombre(
        (nombre) =>
          nombre +
          moment(e.value[0]).format("D") +
          " " +
          moment(e.value[0]).format("MMMM") +
          " " +
          moment(e.value[0]).format("YYYY")
      );
    }
    if (e.value[1]) {
      guardarNombre(
        (nombre) =>
          nombre +
          " - " +
          moment(e.value[1]).format("D") +
          " " +
          moment(e.value[1]).format("MMMM") +
          " " +
          moment(e.value[1]).format("YYYY")
      );
    }
  };
  ...
  const crearPeriodoFuncion = async (e) => {
    ...
    if (fecha[0] === null || fecha[1] === null)
      return mostrarError("Seleccione un rango correcto para el periodo");
    try {
        ...
        <div className="col-9">
          <div className="border pl-2 pr-2 pt-3 mt-4">
            <form onSubmit={crearPeriodoFuncion} className="form-row">
              ...
              <button
                disabled={!nombre || !fecha ? true : false}
                className="ml-2 btn btn-outline-success mb-2"
              >
                <li className="pi pi-plus mr-2" />
                Crear Período
              </button>
            </form>
            {loading ? (
