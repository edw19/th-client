...
function AdministrarPermisos() {
    const { permisos, id: idFuncionario, totalPermisos, funcionario } = useContext(permisosContext);
                                ...
                                <div className="col-9">
                                    <div className="row mt-3 mb-3">
                                        <div className="col ml-3">
                                            <Link to='/dashboard/nuevo-permiso' className="btn btn-link border text-decoration-none">Nuevo Permiso</Link>
                                            {
                                                    ...
                                                    idFuncionario ?
                                                        Object.keys(permisos).length > 0 ?
                                                            permisos.map((permiso) => {
                                                                return (
                                                                    <tr key={permiso.id} style={{ backgroundColor: !permiso.estado && '#fcc162' }}>
                                                                        <td>{moment(new Date(permiso.horaSalida)).format('LL')}</td>
                                                                        <td>{moment(new Date(permiso.horaSalida)).format('HH:mm')}</td>
                                                                        <td>{permiso.horasPermiso + " h y " + permiso.minutosPermiso + " m "} </td>
                                                                        <td>{permiso.motivo}</td>
                                                                        <td>
                                                                            <button className="btn btn-outline-secondary" onClick={() => cambiarEstadoPermiso(permiso.id, permiso.estado)}>
                                                                                {permiso.estado ? 'Contando permiso' : 'Permiso Terminado'}
                                                                            </button>
                                                                        </td>
                                                                        <td>
                                                                            <button disabled={!permiso.estado ? false : true} onClick={() => eliminarPermisoFuncion(permiso.id)} className="btn btn-outline-danger" >Eliminar X</button>
                                                                        </td>
                                                                    </tr>
                                                                )
                                                            }) :
                                                            <tr>
                                                                <td colSpan="6"><SinPermisos /></td>
                                                            </tr>
                                                        : null
                                                }
                                ...
                                <div className="col-3">
                                    ...
                                    <Exito />
                                    {
                                        ...
                                        <>
                                            ...
                                            />
                                            {
                                                funcionario.horasAcumuladas > 0 || funcionario.minutosAcumulados > 0 ?
                                                    <>
